...
__global__ void FD(float *U_prev_d, float *U_new_d, int init, int N,float r, int initG){
...
void launchKernel(float* Ui_prev,float *Ui_next,int offsetL, int tid, int N, float  r, int initG){
    ...
    FD<<<dimGrid, dimBlock>>>(U_prev_d,U_new_d,init,N,r,initG);
...
/*
  ...
  solve: |idnodo|idnodo|solve|id_client|N|U_prev_d|initG|offsetG|r| main->node
...
void distribuir(void* nodos, float *Ui_next, zframe_t* id_client, float* U_prev, int N,float r){
    ...
    paralelizar(U_prev,Ui_next,0,offsetL,N,r);
    ...
    for(int i=1;i<numNodos;i++){
        ...
        for(int j=0;j<N;j++)
            zmsg_addstr(msg,to_string(U_prev[j]).c_str());
...
void handleNodeMessage(zmsg_t* msg, void *nodos){
    ...
    if(code.compare("register")==0){
        ...
        int N = atoi(zmsg_popstr(msg));
        float* U_prev = (float*)malloc(sizeof(float)*N);
        for(int i=0;i<N;i++)
        ...
        omp_set_num_threads(gpus);
        int offsetL=offsetG/gpus;
        paralelizar(U_prev,Ui_next,initG,offsetL,N,r);
        ...
        int initG=atoi(zmsg_popstr(msg));
        int offsetL = atoi(zmsg_popstr(msg)); // Ni
